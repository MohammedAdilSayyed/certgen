    ...
    >
      ...
      <div className="flex items-center justify-between px-16 pt-12 pb-6">
        ...
        <div className="text-center flex-1 px-6">
          ...
          <h1
            ...
            style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: `${bodySize * 2.2}px`, letterSpacing: `${spacing}px` }}
          >
            {data.title}
          </h1>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-16 text-center gap-4">
        ...
        <div className="relative px-12 py-3">
          <h2
            style={{ fontFamily: "'Dancing Script', cursive", fontSize: `${bodySize * 3.5}px`, color: '#7c3d0a', lineHeight: 1.1 }}
          >
            {data.recipientName}
          </h2>
        ...
        <p
          style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: `${bodySize * 1.6}px`, color: '#5a3a0a', fontWeight: 'bold', letterSpacing: '0.1em', textAlign: align }}
        >
          {data.eventName}
        </p>

        <p
          style={{ fontFamily: bodyFont, fontSize: `${bodySize * 0.95}px`, color: '#7a6a4a', maxWidth: '700px', lineHeight: 1.7, textAlign: align, letterSpacing: `${spacing}px` }}
        >
          {data.description}
        </p>

        {data.uniqueId && (
          <p style={{ fontFamily: bodyFont, fontSize: `${bodySize * 0.9}px`, color: '#7a6a4a', letterSpacing: '0.12em', marginTop: 4 }}>
      ...
      <div className="px-16 pb-10 flex items-end justify-between">
        <div className="flex gap-16">
          {data.signatures.map(sig => (
            <SignatureBlock key={sig.id} sig={sig} />
          ))}
        </div>
        {data.qrCodeValue && (
      ...
      <div className="flex-1 flex flex-col px-14 py-10 gap-4">
        ...
        <div style={{ textAlign: align }}>
          <h1
            ...
            style={{ fontSize: `${bodySize * 2.1}px`, letterSpacing: `${spacing}px`, fontFamily: 'system-ui, sans-serif' }}
          >
            {data.title}
          </h1>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center gap-5" style={{ textAlign: align }}>
          ...
          <div className="space-y-2">
            ...
            <p style={{ fontSize: `${bodySize * 1.5}px`, fontWeight: 800, color: '#1e293b', letterSpacing: `${spacing}px` }}>
              {data.eventName}
            </p>
          </div>

          <p style={{ fontSize: `${bodySize * 0.95}px`, color: '#94a3b8', lineHeight: 1.7, maxWidth: 680, letterSpacing: `${spacing}px`, margin: align === 'center' ? '0 auto' : undefined }}>
            {data.description}
          </p>
          ...
          {data.uniqueId && (
            <div className="flex items-center gap-3" style={{ justifyContent: align === 'center' ? 'center' : align === 'right' ? 'flex-end' : 'flex-start' }}>
              <div className="px-4 py-1.5 bg-gray-50 rounded-full">
                <span style={{ fontSize: `${bodySize * 0.85}px`, color: '#64748b', fontWeight: 600, fontFamily: 'monospace' }}>ID: {data.uniqueId}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-end justify-between pt-1">
          <div className="flex gap-14">
            {data.signatures.map(sig => (
              <SignatureBlock key={sig.id} sig={sig} color="indigo" />
            ))}
          </div>
          {data.qrCodeValue && (
            <div className="flex flex-col items-center gap-1">
    ...
    >
        ...
        <div className="text-center">
          ...
          <h1
            ...
            style={{
              ...
              background: 'linear-gradient(to bottom, #f5d572, #c8963a)',
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-12 gap-5" style={{ textAlign: align }}>
        ...
        <div className="relative px-8 py-2">
          <h2
            style={{
              fontFamily: "'Playfair Display', Georgia, serif",
              fontSize: `${bodySize * 3.2}px`,
              background: 'linear-gradient(135deg, #f5d572 0%, #e8b84b 50%, #f5d572 100%)',
              WebkitBackgroundClip: 'text',
              WebkitTextFillColor: 'transparent',
              lineHeight: 1.1,
              letterSpacing: `${spacing}px`
        ...
        <div className="space-y-3">
          <p style={{ fontSize: `${bodySize}px`, color: 'rgba(245,213,114,0.65)', letterSpacing: `${spacing * 0.5 + 1}px` }}>
        ...
        {data.uniqueId && (
          <p style={{ fontSize: `${bodySize * 0.85}px`, color: 'rgba(245,213,114,0.6)', letterSpacing: '0.15em' }}>
            ID: <strong style={{ color: '#f5d572' }}>#{data.uniqueId}</strong>
          </p>
        )}
      </div>
      ...
      <div className="relative z-10 px-16 pb-10 flex items-end justify-between">
        <div className="flex gap-14">
          {data.signatures.map(sig => (
            <SignatureBlock key={sig.id} sig={sig} color="gold" />
          ))}
        </div>
        {data.qrCodeValue && (
          <div className="flex flex-col items-center gap-1">
      ...
      <div className="relative z-10 flex flex-col h-full px-16 py-10 gap-3" style={{ fontFamily: bodyFont }}>
        ...
        <div className="flex items-center justify-between">
          ...
          <h1
            ...
            style={{ fontSize: `${bodySize * 2}px`, letterSpacing: `${spacing}px`, color: '#1e293b' }}
          >
            {data.title}
          </h1>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4" style={{ textAlign: align }}>
          ...
          <h2 style={{
            ...
            fontSize: `${bodySize * 3.2}px`, color: '#1e3a8a', lineHeight: 1
          }}>
            {data.recipientName}
          </h2>
          <p style={{ fontSize: `${bodySize}px`, color: '#6b7280', letterSpacing: `${spacing}px` }}>
            has successfully completed
          </p>
          <p style={{ fontSize: `${bodySize * 1.5}px`, fontWeight: 700, color: '#1e293b', letterSpacing: `${spacing}px` }}>
            {data.eventName}
          </p>
          <p style={{ fontSize: `${bodySize * 0.9}px`, color: '#9ca3af', lineHeight: 1.7, maxWidth: 680, letterSpacing: `${spacing}px` }}>
            {data.description}
          </p>
          {data.uniqueId && (
            <p style={{ fontSize: `${bodySize * 0.85}px`, color: '#6b7280' }}>
